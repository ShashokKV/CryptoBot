...
class PairsAdapter(pairsHolder: ContextHolder) : RecyclerViewAdapter<PairsViewHolder>(pairsHolder) {
    ...
    override fun onBindViewHolder(holder: PairsViewHolder, position: Int) {
        ...
        holder.bittrextBidView.text = String.format(Locale.getDefault(), "%.8f", pair.bittrexBid)
        holder.bittrextBidQuantityView.text = String.format(Locale.getDefault(), "%.8f", pair.bittrexBidQuantity)
    ...
    inner class PairsViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val bittrextBidView: TextView = itemView.findViewById(R.id.bittrex_bid)
        val bittrextBidQuantityView: TextView = itemView.findViewById(R.id.bittrex_bid_quantity)
