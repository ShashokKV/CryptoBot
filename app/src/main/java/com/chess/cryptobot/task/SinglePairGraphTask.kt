...
class SinglePairGraphTask(
    ...
    private fun addToEntriesGroup(
        entriesGroups: List<MutableList<BarEntry>>,
        pair: ProfitPair?,
        barEntryIndex: Int
    ) {
        pair?.percent?.let { BarEntry(barEntryIndex.toFloat(), it) }
            ?.let { entriesGroups[0].add(it) }
    }
